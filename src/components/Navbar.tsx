...
export default function Navbar(props: any) {
            ...
            try {
                ...
                if (data.items && Array.isArray(data.items) && data.items.length > 0) {
                    ...
                    if (lastSeen === 0) {
                        ...
                        const newCount = data.items.filter((item: any) => {
    ...
    useEffect(() => {
        const checkUpcomingExams = async () => {
            try {
                ...
                const hasExams = events.some((event: any) => {
        ...
        const handleScroll = () => {
            if (!ticking) {
                window.requestAnimationFrame(() => {
                    ...
                    if (scrollY > 91 && !isScrolled) {
                        setIsScrolled(true);
                    } else if (scrollY < 68 && isScrolled) {
                        setIsScrolled(false);
                    }
    ...
    const handleLongPressStart = (buttonId: keyof typeof showTooltip) => {
    ...
    const handleLongPressEnd = (buttonId: keyof typeof showTooltip) => {
        if (longPressTimer) {
    ...
    const handleClick = (buttonId: keyof typeof showTooltip) => {
        setShowTooltip(prev => ({...prev, [buttonId]: false}));
                ...
                {!isExamensPage && (
                    <div className="navbar-controls">
                        ...
                        {showFullYear && (
                            <div className="week-picker-container">
                                <button
                                    onClick={onToggleFullYear as any}
                                    style={{
                                ...
                                <div className="view-filter-group">
                                            ...
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    ...
                                                    <g fill="none" strokeMiterlimit="10" fontFamily="none"
                                                       ...
                                                       textAnchor="middle" transform="scale(5.33333)">
                                                        <path fill="url(#a)"
                                    ...
                                    {onToggleFullYear && (
                                        ...
                                        >
                                            <button
                                                type="button"
                                                className={`view-filter-group-btn view-filter-group-btn--full-year ${showFullYear ? 'active' : ''}`}
                                                onClick={() => {
                                                ...
                                                onMouseLeave={() => setShowTooltip(prev => ({
                                                    ...prev,
                                                    fullYear: false
                                                }))}
                                            ...
                                            >
                                                <span className="navbar-full-year-label" aria-hidden="true">
                                                    {String(calendarYear).split('').map((digit, i) => (
                                                        <span key={`y-${calendarYear}-${i}`}
                                                              className="navbar-full-year-digit">
                                                            {digit}
                                                        </span>
                                                    ))}
                                                </span>
                                                ...
                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                                                    ...
                                                    {viewMode === 'horizontal' ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <rect x="4" y="9" width="4.5" height="11" rx="0.5"
                                                                  fill="#60a5fa" fillOpacity="0.6" stroke="#2563eb"
                                                                  strokeWidth="1" opacity="0.9"/>
                                                            <rect x="9.75" y="9" width="4.5" height="11" rx="0.5"
                                                                  fill="#4ade80" fillOpacity="0.6" stroke="#16a34a"
                                                                  strokeWidth="1" opacity="0.9"/>
                                                            <rect x="15.5" y="9" width="4.5" height="11" rx="0.5"
                                                                  fill="#ef4444" fillOpacity="0.85" stroke="#dc2626"
                                                                  strokeWidth="1" opacity="0.9"/>
                                                        </>
                                                    ) : (
                                                        ...
                                                        <>
                                                            ...
                                                            <line x1="9" y1="12" x2="20" y2="12" stroke="currentColor"
                                                                  strokeWidth="1.5" strokeLinecap="round"/>
                                                            <rect x="4" y="16" width="3" height="4" rx="0.5"
                                                                  fill="#ef4444" fillOpacity="0.85" stroke="#dc2626"
                                                                  strokeWidth="1"/>
                                    ...
                                    >
                                        <div style={{position: "relative"}}>
                                            <button
                                                ...
                                                onMouseEnter={() => setShowTooltip(prev => ({
                                                    ...prev,
                                                    history: true
                                                }))}
                                                onMouseLeave={() => setShowTooltip(prev => ({
